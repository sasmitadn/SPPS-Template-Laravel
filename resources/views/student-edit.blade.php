...
@section('content')
    <div class="">
        ...
        <form action="{{ route('student.update', ['id' => $data->code]) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="code" value="{{ old('code', $data->code) }}" disabled>
                @error('code')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="class" value="{{ old('class', $data->class) }}">
                @error('class')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
                            ...
                            <tr>
                                <td>
                                    <input type="checkbox" name="invoices[]" value="{{ $invoice->id }}"
                                        {{ in_array($invoice->id, old('invoices', json_decode($savedInvoices, true) ?? [])) ? 'checked' : '' }}>
                                </td>
                                <td>{{ $invoice->title }}</td>
            ...
            <button class="btn btn-primary my-5" type="submit">Update Sekarang</button>
        </form>
    </div>
@endsection
